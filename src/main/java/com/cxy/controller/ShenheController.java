...
public class ShenheController {
  ...
  public boolean deletePage(@RequestBody JSONObject userArray) {
    ...
    boolean isDeletes = shenheMapper.deleteMoreUsers(idLists);
   ...
   */
  @PostMapping("/getOneShenheInfo")
  public List<Shenhe> getOneUserInfo(@RequestBody JSONObject keywords) {
    // 前端获取的关键字信息参数
    ...
    List<Shenhe> userList = shenheMapper.selectOneUser(keyName,formatStartTime,formatEndTime);
    // 用数组来将对象转起来
    return userList;
  }
   ...
   */
  @PostMapping("/getAllShenheInfo")
  public Message13 getAllUserInfo(@RequestBody JSONObject requestParm) {
    // 装信息的工具类
    ...
    List<Shenhe> userList = shenheMapper.selectAllUsers();
    for(Shenhe questionnaire: userList){
      System.out.println(questionnaire);
    }
    PageInfo<Shenhe> pageInfo = new PageInfo<>(userList);
