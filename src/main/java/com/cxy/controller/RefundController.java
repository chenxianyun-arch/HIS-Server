...
public class RefundController {
  ...
  public boolean deletePage(@RequestBody JSONObject userArray) {
    ...
    boolean isDeletes = refundDao.deleteMoreUsers(idLists);
   ...
   */
  @PostMapping("/getOneRefundInfo")
  public List<Refund> getOneUserInfo(@RequestBody JSONObject keywords) {
    // 前端获取的关键字信息参数
    ...
    List<Refund> userList = refundDao.selectOneUser(keyName,formatStartTime,formatEndTime);
    // 用数组来将对象转起来
    return userList;
  }
   ...
   */
  @PostMapping("/getAllRefundInfo")
  public Message7 getAllUserInfo(@RequestBody JSONObject requestParm) {
    // 装信息的工具类
    ...
    List<Refund> userList = refundDao.selectAllUsers();
    for(Refund questionnaire: userList){
      System.out.println(questionnaire);
    }
    PageInfo<Refund> pageInfo = new PageInfo<>(userList);
