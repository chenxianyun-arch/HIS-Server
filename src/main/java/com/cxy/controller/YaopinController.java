...
import com.cxy.dao.YaopinMapper;
import com.cxy.pojo.Jiesuan;
import com.cxy.pojo.Yaopin;
import com.cxy.utils.Message12;
...
public class YaopinController {
  ...
  public boolean deletePage(@RequestBody JSONObject userArray) {
    ...
    boolean isDeletes = yaopinMapper.deleteMoreUsers(idLists);
   ...
   */
  @PostMapping("/getOneYaopinInfo")
  public List<Yaopin> getOneUserInfo(@RequestBody JSONObject keywords) {
    // 前端获取的关键字信息参数
    ...
    List<Yaopin> userList = yaopinMapper.selectOneUser(keyName,formatStartTime,formatEndTime);
    // 用数组来将对象转起来
    return userList;
  }
   ...
   */
  @PostMapping("/getAllYaopinInfo")
  public Message12 getAllUserInfo(@RequestBody JSONObject requestParm) {
    // 装信息的工具类
    ...
    List<Yaopin> userList = yaopinMapper.selectAllUsers();
    for(Yaopin questionnaire: userList){
      System.out.println(questionnaire);
    }
    PageInfo<Yaopin> pageInfo = new PageInfo<>(userList);
