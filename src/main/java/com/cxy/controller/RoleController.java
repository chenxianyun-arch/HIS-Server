...
public class RoleController {
  ...
  public boolean deletePage(@RequestBody JSONObject userArray) {
    ...
    boolean isDeletes = roleMapper.deleteMoreUsers(idLists);
  ...
  public List<Role> getOneUserInfo(@RequestBody JSONObject keywords) {
    // 前端获取的关键字信息参数
    ...
    List<Role> userList = roleMapper.selectOneUser(keyName,formatStartTime,formatEndTime);
    // 用数组来将对象转起来
    return userList;
  }
  ...
  public Message15 getAllUserInfo(@RequestBody JSONObject requestParm) {
    // 装信息的工具类
    Message15 message = new Message15();
    // 当前页码 每页显示页数
    Integer currentPage = requestParm.getInteger("currentPage");
    Integer pageSize = requestParm.getInteger("pageSize");
    // 利用插件进行分页
    PageHelper.startPage(currentPage,pageSize);
    List<Role> userList = roleMapper.selectAllUsers();
    for(Role questionnaire: userList){
