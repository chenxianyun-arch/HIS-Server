 ...
 */
@RestController
public class WenzhenController {
  ...
  public boolean deletePage(@RequestBody JSONObject userArray) {
    ...
    boolean isDeletes = wenzhenDao.deleteMoreUsers(idLists);
   ...
   */
  @PostMapping("/getOneWenzhenInfo")
  public List<Wenzhen> getOneUserInfo(@RequestBody JSONObject keywords) {
    // 前端获取的关键字信息参数
    ...
    List<Wenzhen> userList = wenzhenDao.selectOneUser(keyName,formatStartTime,formatEndTime);
    // 用数组来将对象转起来
    return userList;
  }
   ...
   */
  @PostMapping("/getAllWenzhenInfo")
  public Message4 getAllUserInfo(@RequestBody JSONObject requestParm) {
    // 装信息的工具类
    ...
    List<Wenzhen> userList = wenzhenDao.selectAllUsers();
    for(Wenzhen questionnaire: userList){
      System.out.println(questionnaire);
    }
    PageInfo<Wenzhen> pageInfo = new PageInfo<>(userList);
