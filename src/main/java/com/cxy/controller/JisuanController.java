...
import com.cxy.dao.JiesuanDao;
import com.cxy.pojo.Jiesuan;
import com.cxy.utils.Message9;
...
public class JisuanController {
  ...
  public boolean deletePage(@RequestBody JSONObject userArray) {
    ...
    boolean isDeletes = jiesuanDao.deleteMoreUsers(idLists);
   ...
   */
  @PostMapping("/getOneJiesuanInfo")
  public List<Jiesuan> getOneUserInfo(@RequestBody JSONObject keywords) {
    // 前端获取的关键字信息参数
    ...
    List<Jiesuan> userList = jiesuanDao.selectOneUser(keyName,formatStartTime,formatEndTime);
    // 用数组来将对象转起来
    return userList;
  }
   ...
   */
  @PostMapping("/getAllJiesuanInfo")
  public Message9 getAllUserInfo(@RequestBody JSONObject requestParm) {
    // 装信息的工具类
    Message9 message = new Message9();
    // 当前页码 每页显示页数
    Integer currentPage = requestParm.getInteger("currentPage");
    Integer pageSize = requestParm.getInteger("pageSize");
    // 利用插件进行分页
    PageHelper.startPage(currentPage,pageSize);
    List<Jiesuan> userList = jiesuanDao.selectAllUsers();
    for(Jiesuan questionnaire: userList){
      System.out.println(questionnaire);
    }
    PageInfo<Jiesuan> pageInfo = new PageInfo<>(userList);
