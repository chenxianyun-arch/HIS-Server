...
public class ReceivedController {
  ...
  public boolean deletePage(@RequestBody JSONObject userArray) {
    ...
    boolean isDeletes = receivedDao.deleteMoreUsers(idLists);
   ...
   */
  @PostMapping("/getOneReceivedInfo")
  public List<Received> getOneUserInfo(@RequestBody JSONObject keywords) {
    // 前端获取的关键字信息参数
    ...
    List<Received> userList = receivedDao.selectOneUser(keyName,formatStartTime,formatEndTime);
    // 用数组来将对象转起来
    return userList;
  }
   ...
   */
  @PostMapping("/getAllReceivedInfo")
  public Message10 getAllUserInfo(@RequestBody JSONObject requestParm) {
    // 装信息的工具类
    ...
    List<Received> userList = receivedDao.selectAllUsers();
    for(Received questionnaire: userList){
      System.out.println(questionnaire);
    }
    PageInfo<Received> pageInfo = new PageInfo<>(userList);
